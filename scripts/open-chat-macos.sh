...
tell application "Terminal"
  activate
  set newWindow to do script "cd '$ROOT' && if ! /usr/bin/pgrep -f '[t]hree-headed-snake-orchestrator.py' >/dev/null 2>&1; then /usr/bin/nohup bash scripts/start-orchestrator.sh >>logs/orchestrator.log 2>&1 & echo '[orchestrator started]'; else echo '[orchestrator already running]'; fi; bash scripts/chat.sh --replay 30 --poll-ms 75 --no-auto-drain --window-token '$WINDOW_TOKEN'"
