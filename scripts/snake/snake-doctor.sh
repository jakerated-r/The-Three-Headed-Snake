...
GEMINI_MODEL="${SNAKE_GEMINI_MODEL:-gemini-2.5-flash-lite}"
echo "-- Gemini  / gemini : $(classify "$(env GOOGLE_GENAI_USE_GCA=true GOOGLE_CLOUD_ACCESS_TOKEN=DO_NOT_USE "$GEMINI_HEAD" -p 'reply with the single word OK' -m "$GEMINI_MODEL" --approval-mode plan --output-format text --skip-trust --extensions '' 2>&1 | head -5)")"
